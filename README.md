...
3. Ensure **`stackbit.config.ts`** is on the default branch Netlify reads; it defines `GitContentSource` for `src/content/` (project root via **`process.cwd()`** so the bundled CJS config on Netlify does not rely on `import.meta.url`), a **`sitemap`** that maps every Git document to `/` (SPA), and a **custom Vite** dev command with HMR passthrough (`/vite-hmr/**`).
